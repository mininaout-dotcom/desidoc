...
const GENERATION_PROMPT = [
  ...
  "- Брифинг и сбор данных (2-4 ч): интервью с заказчиком, сбор вводных, задачи, ограничения, критерии успеха.",
  ...
  "- Визуальная концепция (4-12 ч): стиль, палитра, типографика, графические приёмы под проект.",
  ...
  "- Препресс и подготовка файлов к передаче (2-8 ч): вылеты, цветовой режим, печатный PDF; многостраничные издания — больше.",
  ...
  "2. Часы каждого этапа — строго в диапазоне из меню. Для этапов вне меню — не больше 12 часов.",
  "3. Исследование/анализ и концепция/мудборд — всегда ДВА отдельных этапа, не объединяй их.",
  "4. Первый этап — всегда брифинг; концепцию и дизайн в него включать нельзя.",
  "5. Обязательны: этап подготовки файлов (для печати — «Препресс и подготовка файлов к передаче») и «Согласование и правки».",
  ...
  "",
  "ОСОБЫЕ СЛУЧАИ:",
  "- Упаковка: Брифинг и сбор данных (3 ч) → Исследование и стратегия → Мудборд и референсы → Дизайн коробки (внешняя и внутренняя части — один этап) → отдельные материалы (инструкция, вкладыш, стикерпак) → препресс → правки.",
...
function normalizeAnalysis(value, sourceText = "") {
  ...
  const adjustedStages = applyPrintMinimumHours(
    clampStageHours(postProcessStages(stages, projectType, sourceText)),
    projectType,
    sourceText
  );
