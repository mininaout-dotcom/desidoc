...
const PROFILE_STORAGE_KEY = "designkit.unitPricingPrototype.profile";
const ESTIMATE_META_STORAGE_KEY = "designkit.unitPricingPrototype.estimateMeta";
const COOKIE_CONSENT_STORAGE_KEY = "designkit.unitPricingPrototype.cookieConsent";
...
const state = {
  ...
  rateMode: "custom",
  marketGrade: "middle",
  designerRate: Number(localStorage.getItem("designkit.unitPricingPrototype.designerRate") || DEFAULT_DESIGNER_RATE),
  monthlyIncome: Number(localStorage.getItem("designkit.unitPricingPrototype.monthlyIncome") || 240000),
  workDays: Number(localStorage.getItem("designkit.unitPricingPrototype.workDays") || 5),
  billableHours: Number(localStorage.getItem("designkit.unitPricingPrototype.billableHours") || 6),
  currency: CURRENCIES[localStorage.getItem("designkit.unitPricingPrototype.currency")] ? localStorage.getItem("designkit.unitPricingPrototype.currency") : DEFAULT_CURRENCY,
  rate: 0,
  taxMode: "selfEmployed",
  clientType: "individual",
  customTaxRate: Number(localStorage.getItem("designkit.unitPricingPrototype.customTaxRate") || 6),
  generated: false,
  stages: [],
  expenses: [],
  mods: new Set(),
  exportShowHours: localStorage.getItem("designkit.unitPricingPrototype.exportShowHours") !== "false",
  exportHideBranding: localStorage.getItem("designkit.unitPricingPrototype.exportHideBranding") === "true",
...
function createStages(project, level) {
  return (project.stages || []).map((stage) => {
    ...
    return {
      ...
      // Зашитая цена за единицу (300/500 ₽) не пересчитывается при смене грейда
...
function setTheme(theme) {
  ...
  localStorage.setItem("designkit.unitPricingPrototype.theme", state.theme);
}
...
function saveEstimate() {
  const saves = JSON.parse(localStorage.getItem("designkit.unitPricingPrototype.estimates") || "[]");
  saves.unshift({
  ...
  localStorage.setItem("designkit.unitPricingPrototype.estimates", JSON.stringify(saves.slice(0, 12)));
...
function printEstimate() {
  ...
  localStorage.setItem("designkit.unitPricingPrototype.profile", JSON.stringify(state.profile));
  persistEstimateMeta();
  renderGreeting();
...
function loadContractDraft() {
  let saved = null;
  try {
    saved = JSON.parse(localStorage.getItem("designkit.unitPricingPrototype.contractDraft") || "null");
  } catch (error) {
    localStorage.removeItem("designkit.unitPricingPrototype.contractDraft");
  }
...
function scheduleContractSave() {
  ...
  contractSaveTimer = setTimeout(() => {
    ...
    try {
      localStorage.setItem("designkit.unitPricingPrototype.contractDraft", JSON.stringify(contractState));
      updateAutosaveStatus("Сохранено");
    } catch (e) {
...
function bindEvents() {
  document.addEventListener("click", (event) => {
    ...
    if (currencyTarget) {
      ...
      localStorage.setItem("designkit.unitPricingPrototype.currency", state.currency);
      renderRate();
      if (state.generated) updateTotalsOnly();
      return;
    }
    ...
    if (action === "duplicate-contract") {
      localStorage.setItem(`designkit.unitPricingPrototype.contractDraft.${Date.now()}`, JSON.stringify(contractState));
      updateAutosaveStatus("Дубликат сохранён");
    }
    if (action === "archive-contract") {
      localStorage.setItem(`designkit.unitPricingPrototype.contractArchive.${Date.now()}`, JSON.stringify(contractState));
  ...
  document.addEventListener("input", (event) => {
    ...
    if (input.matches('[data-input="designerRate"]')) {
      ...
      localStorage.setItem("designkit.unitPricingPrototype.designerRate", String(state.designerRate));
      renderRate();
    ...
    if (input.matches('[data-input="monthlyIncome"], [data-input="workDays"], [data-input="billableHours"]')) {
      ...
      localStorage.setItem(`designkit.unitPricingPrototype.${key}`, String(state[key]));
      renderRate();
    ...
    if (input.matches('[data-input="customTaxRate"]')) {
      ...
      localStorage.setItem("designkit.unitPricingPrototype.customTaxRate", String(state.customTaxRate));
      renderRate();
      if (state.generated) updateTotalsOnly();
    }
  ...
  document.addEventListener("change", (event) => {
    ...
    if (input.matches("[data-export-hours-toggle]")) {
      ...
      localStorage.setItem("designkit.unitPricingPrototype.exportShowHours", String(state.exportShowHours));
      renderExportOptions();
    ...
    if (input.matches("[data-export-branding-toggle]")) {
      ...
      localStorage.setItem("designkit.unitPricingPrototype.exportHideBranding", String(state.exportHideBranding));
      renderExportOptions();
      return;
    }
...
function randomAvatar() {
  ...
  if (!btn) return;
  const stored = sessionStorage.getItem("designkit.unitPricingPrototype.avatarEmoji");
  if (stored) { btn.textContent = stored; return; }
  const emoji = AVATAR_ANIMALS[Math.floor(Math.random() * AVATAR_ANIMALS.length)];
  btn.textContent = emoji;
  try { sessionStorage.setItem("designkit.unitPricingPrototype.avatarEmoji", emoji); } catch(e) {}
}
